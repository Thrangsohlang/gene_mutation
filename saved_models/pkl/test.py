...
# Function to check if Gene and Variation exist in the encoded data


# MongoDB connection setup
client = pymongo.MongoClient("mongodb://localhost:27017/")
my_db = client["my_database"]  # connecting to my_database
gene_collection = my_db["genes"]  # collection genes
variation_collection = my_db["variations"]  # collection variation

# Function to check if Gene and Variation exist in MongoDB collections
def check_gene_variation_existence(gene, variation):
    gene_exists = gene_collection.count_documents({"Gene": gene}) > 0
    variation_exists = variation_collection.count_documents({"Variation": variation}) > 0
    return gene_exists and variation_exists
...
if st.button('Predict'):
    # Check if Gene and Variation exist in the MongoDB collections
    if check_gene_variation_existence(gene_input, variation_input):
        ...
        st.write("Gene or Variation not found!. Please enter valid values")
